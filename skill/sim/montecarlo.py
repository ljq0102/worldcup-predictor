...
def run(model, fixtures: pd.DataFrame, n: int = 50000, seed: int = 0,
    ...
    # --- 收集每场比赛的所有模拟比分 ---
    scoreline_counter = defaultdict(Counter)
    ...
    for r in fixtures.itertuples():
        ...
        # 记录每场模拟的比分
        for i in range(n):
            scoreline_counter[(r.home_team, r.away_team)][f"{hg[i]}-{ag[i]}"] += 1
    ...
    # --- 统计每场比赛 top-3 比分 ---
    match_scorelines = {}
    for (home, away), counter in scoreline_counter.items():
        total = sum(counter.values())
        match_scorelines[f"{home} vs {away}"] = [
            {"score": s, "probability": round(c / total, 4)}
